...
## 8. CASCADE削除時のカウント不整合（Issue #8）

### 何が起きた？
返信付きの親コメントを削除すると、コメント数が1しか減らなかった。例えば親1件+返信2件=3件を削除したのに、カウントは3→2にしかならなかった（正しくは3→0）。

### 原因
- DBの外部キー制約で `ON DELETE CASCADE` が設定されており、親コメント削除時に返信も自動削除される
- しかしAPI側では `comments_count` を常に `-1` しか減らしていなかった
- フロントエンドでも親コメントだけUIから除去し、返信はそのまま残っていた

### 壊れていたコード
```typescript
// API側: 常に-1（CASCADEで3件消えても-1）
await supabase
  .from("memos")
...
### 防止策
- CASCADE削除がある場合、カウントを手動で計算せず、**削除後にDBから実数を取得**する
- `SELECT count(*) FROM comments WHERE memo_id = ?` のように、DBの実際の件数で上書きする
- CASCADE設定のあるテーブルでは「何件連鎖削除されるか」を常に意識する
- フロントエンドでもAPIから返された正確なカウントを使う
...
### データベース
- [ ] CASCADE削除のあるテーブルで、カウント更新が連鎖削除分を考慮しているか
- [ ] カウント系カラムは手動計算ではなくDBの実数を取得しているか
